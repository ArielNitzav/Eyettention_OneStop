...
if __name__ == '__main__':
	...
	onestop = {"model_pretrained": "bert-base-cased",
			...
			"max_sp_len": 675, # max number of words in a scanpath, include start token and end token
			"max_sp_token": 850, # maximum number of tokens a scanpath includes. include start token and end token
			"norm_type": 'z-score',
	...
	# CELER: sn_list = np.unique(word_info_df[word_info_df['list'].isin(reader_list)].sentenceid.values).tolist()
	sn_list = np.unique(word_info_df.article_paragraph_id.values).tolist()
