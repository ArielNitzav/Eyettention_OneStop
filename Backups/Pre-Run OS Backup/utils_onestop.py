...
def load_corpus(corpus, task=None): # V
	if corpus == 'BSC':
		...
		eyemovement_df = pd.read_csv('../OneStop-Eye-Movements/ordinary/fixations_P_ordinary.csv')
		...
		eyemovement_df['article_paragraph_id'] = eyemovement_df['article_id'].astype(str) + '_' + eyemovement_df['paragraph_id'].astype(str) # create article_paragraph IDs
		# eyemovement_df['CURRENT_FIX_INTEREST_AREA_LABEL'] = eyemovement_df.CURRENT_FIX_INTEREST_AREA_LABEL.replace('\t(.*)', '', regex=True)
		word_info_df = pd.read_csv('../OneStop-Eye-Movements/ordinary/ia_P_ordinary.csv')
		...
		word_info_df['article_paragraph_id'] = word_info_df['article_id'].astype(str) + '_' + word_info_df['paragraph_id'].astype(str) # create article_paragraph IDs
		return word_info_df, None, eyemovement_df
...
def _process_onestop(sn_list, reader_list, word_info_df, eyemovement_df, tokenizer, cf):
	...
	for sn_id in tqdm(sn_list):
		#process sentence sequence
		sn_df = eyemovement_df[eyemovement_df.article_paragraph_id==sn_id]
		#notice: Each sentence is recorded multiple times in file |word_info_df|.
		sn = word_info_df[word_info_df.article_paragraph_id == sn_id]
		sn = sn[sn['participant_id']==sn.participant_id.values.tolist()[0]] # between 0 to 60
		#compute word length for each word
		sn_word_len = compute_word_length_onestop(sn.word_length.values)

		sn_str = sn.paragraph.iloc[-1]
		...
		#process fixation sequence
		for sub_id in reader_list:
			sub_df = sn_df[sn_df.participant_id==sub_id]
			...
			sp_token = [sn_str.split()[int(i)] for i in sp_ordinal_pos] ########################## sn_str.split() is shorter than sp_ordinal_pos
			sp_token_str = '[CLS]' + ' ' + ' '.join(sp_token) + ' ' + '[SEP]'
