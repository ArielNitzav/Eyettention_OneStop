...
def process_dataset(fold_dict, fold_sub_dict, fold_indices, sn_list):
	fold_articles = [fold_dict[f] for f in fold_indices]
	fold_articles = [item for items in fold_articles for item in items]

	fold_subs = [fold_sub_dict[f] for f in fold_indices]
	fold_subs = [item for items in fold_subs for item in items]
		
	sn_list = [sn for sn in sn_list if '_'.join(sn.split('_')[:2]) in fold_articles]
	reader_list = [int(sub.split('_')[1]) for sub in fold_subs]

	return sn_list, reader_list
...
def load_corpus(corpus, include_repeated_reading=False, has_preview=False):
	if corpus == 'celer':
		...
		eyemovement_df = pd.read_csv('/data/home/shared/onestop/OneStop_v1_20250126/lacclab_processed/fixations_Paragraph.csv')
		...
		word_info_df = pd.read_csv('/data/home/shared/onestop/OneStop_v1_20250126/lacclab_processed/ia_Paragraph.csv')
...
def ez_reader_formatter(unique_paragraph_ids, text_spacing_version, cls_dnn_sp, reg_dnn_sp, fold):
	...
	for i, (upi, tsv, cds, rds) in enumerate(zip(unique_paragraph_ids, text_spacing_version, cls_dnn_sp, reg_dnn_sp)):
		...
		df_dict["sp_fix_pos"] += cds.tolist()
		df_dict["sp_fix_dur"] += (rds*1000).tolist()
...
def _process_onestop(sn_list, reader_list, word_info_df, eyemovement_df, tokenizer, cf, check_baseline, keep_ids):
	"""
...
class onestop_dataset(Dataset):
	...
	def __init__(
		self,
		word_info_df, eyemovement_df, cf, reader_list, sn_list, tokenizer, check_baseline=False, keep_ids=False,
	):

		self.data = _process_onestop(sn_list, reader_list, word_info_df, eyemovement_df, tokenizer, cf, check_baseline, keep_ids)
