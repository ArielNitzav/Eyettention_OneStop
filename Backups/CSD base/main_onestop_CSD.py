...
if __name__ == '__main__':
	...
	onestop = {"model_pretrained": "bert-base-cased",
			...
			"train_batch_size": 64,
			"val_batch_size": 16,
			"test_batch_size": 512, # large number, to make sure everything fits in one batch
	...
	fold_dict, fold_sub_dict = load_fold_subjects()
	...
	for fold_indx in range(n_folds):
		...
		fold_test_indices, fold_validation_indices, fold_train_indices = get_fold_indices(fold_indx)

		sn_list_train, reader_list_train = process_dataset(fold_dict, fold_sub_dict, fold_train_indices[:2], sn_list)#############################################
		sn_list_val, reader_list_val = process_dataset(fold_dict, fold_sub_dict, fold_validation_indices, sn_list)
		sn_list_test, reader_list_test = process_dataset(fold_dict, fold_sub_dict, fold_test_indices, sn_list)
		...
		dataset_test = onestop_dataset(word_info_df, eyemovement_df, onestop, reader_list_test, sn_list_test, tokenizer, check_baseline=False, keep_ids=keep_test_ids)
		...
		for episode_i in range(episode, 1): #onestop["n_epochs"]+1):
			if inference_mode:
				break
			...
			with torch.no_grad():
				...
				if bool(args.scanpath_gen_flag) == True:
					...
					if keep_test_ids:
						...
						res = ez_reader_formatter(unique_paragraph_ids, text_spacing_version, cls_sp_dnn, reg_sp_dnn, fold_indx)
						print()
