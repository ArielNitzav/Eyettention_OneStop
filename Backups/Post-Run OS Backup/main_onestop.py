...
if __name__ == '__main__':
	...
	for train_idx, test_idx in kf.split(split_list):
		loss_dict = {'val_loss':[], 'train_loss':[], 'test_ll':[], 'test_NLD': [],'test_AUC':[]}
		...
		dataset_test = onestop_dataset(word_info_df, eyemovement_df, onestop, reader_list_test, sn_list_test, tokenizer)
		...
		for episode_i in range(episode, 5):#onestop["n_epochs"]+1):
			dnn.train()
			...
			for batchh in train_dataloaderr:
				...
				print('Batch {}\tAverage Error: {:.10f} '.format(counter, np.mean(av_score)), end=" ")
			...
			if np.mean(val_loss) < old_score:
				...
				torch.save(dnn.state_dict(), '{}/CELoss_OneStop_{}_eyettention_{}_newloss_fold{}.pth'.format(args.save_data_folder, args.test_mode, args.atten_type, fold_indx))
		...
		res_llh = []
		res_NLD = []
		dnn.load_state_dict(torch.load(os.path.join(args.save_data_folder,f'CELoss_OneStop_{args.test_mode}_eyettention_{args.atten_type}_newloss_fold{fold_indx}.pth'), map_location='cpu'))
		dnn.to(device)
		batch_indx = 0
		for batchh in test_dataloaderr:
			with torch.no_grad():
				...
				if bool(args.scanpath_gen_flag) == True:
					...
					sp_dnn, sp_human = prepare_scanpath(sp_dnn.detach().to('cpu').numpy(), sn_len, sp_pos_test, onestop)
					sp_dnn_list.extend(sp_dnn)
					sp_human_list.extend(sp_human)
					res_NLD.append(NLD(sp_human, sp_dnn))
