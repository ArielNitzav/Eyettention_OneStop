...
class ResidualBlock(nn.Module):
	...
	def forward(self, x):
		return x + self.linear2(self.relu(self.linear1(x)))  # Skip connection
...
class Eyettention(nn.Module):
	def __init__(self, cf):
		...
		self.cls_decoder_dense = nn.Sequential(
							 self.dropout_dense,
		...
		'''
		self.reg_decoder_dense = nn.Sequential(
							self.dropout_dense,
							nn.Linear(self.hidden_size * 2 + 1, 512),
							nn.ReLU(),
							ResidualBlock(512, 256),  # Residual connection
							self.dropout_dense,
							ResidualBlock(512, 128),  # Another residual connection
							nn.Linear(512, 1)
		)
		'''
		...
		self.reg_decoder_dense = nn.Sequential(
							 ...
							 nn.Linear(256, 256),
							 nn.ReLU(),
							 nn.Linear(256, 1), #number of output classes
		)
	...
	def decode(self, sp_emd, sn_mask, sp_pos, enc_out, sp_fix_dur, sp_landing_pos, word_ids_sp):
		...
		for i in range(dec_emb_in.shape[0]):
			...
			# Classification
			cls_result = self.cls_decoder_dense(hc)                      # [batch, dec_o_dim]
			cls_output.append(cls_result)
			
			# Regression
			reg_result = self.reg_decoder_dense(hc)                      # [batch, dec_o_dim]
			reg_output.append(reg_result)


		cls_output = torch.stack(cls_output, dim=0)                     # [step, batch, dec_o_dim]
		reg_output = torch.stack(reg_output, dim=0)                     # [step, batch, dec_o_dim]
		
		#output = F.softmax(output, dim=2) # cross entropy in pytorch includes softmax
		return cls_output.permute(1,0,2), reg_output.permute(1,0,2), atten_weights_batch                           # [batch, step, dec_o_dim]
	...
	def forward(self, sn_emd, sn_mask, sp_emd, sp_pos, word_ids_sn, word_ids_sp, sp_fix_dur, sp_landing_pos, sn_word_len):
		...
		if sn_mask_word is None:#for Chinese dataset without token pooling
			...
			cls_pred, reg_pred, atten_weights = self.decode(sp_emd,
												sn_mask_word,
												sp_pos,
												x,
												sp_fix_dur,
		...
		return cls_pred, reg_pred, atten_weights
	...
	def scanpath_generation(self, sn_emd,
		...
		cls_output.append(start_pos.long())
		reg_output.append(start_pos.long())
		for p in range(max_pred_len-1):
			...
			cls_result = self.softmax(cls_result) # [batch, dec_o_dim]
			density_prediction.append(cls_result)
			...
			pred_indx = torch.multinomial(cls_result, 1)
			pred_class = [le.classes_[pred_indx[i]] for i in torch.arange(cls_result.shape[0])]
			...
			pred_pos = cls_output[-1] + pred_class
			...
			cls_output.append(pred_pos)
			reg_output.append(reg_result.squeeze())
			...
			#add positional embeddings
			position_embeddings = self.position_embeddings(cls_output[-1])
			dec_emb_in = dec_emb_in + position_embeddings
			dec_emb_in = self.LayerNorm(dec_emb_in)

			#concatenate two additional gaze features: landing_pos (zeros), fix_duration
			dec_in = torch.cat((dec_emb_in, torch.zeros(dec_emb_in.shape[0], 1).to(sn_emd.device), reg_result), dim=1)

		cls_output = torch.stack(cls_output, dim=0).permute(1, 0) # [batch, step]
		reg_output = torch.stack(reg_output, dim=0).permute(1, 0) # [batch, step]
